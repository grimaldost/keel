...
|---|---|---|---|
| **DoR gate** | spec-readiness check before decompose | `definition-of-ready.md` + `keel check-ready` | `./bin/keel check-ready spec-*.md` |
...
| **Edit-time invariant hook** | block edits that violate a boundary | `plugins/acme-contributor/hooks/pre-edit-boundary.py` | none (contract.py is the only invariant boundary; manual review enforces) |
