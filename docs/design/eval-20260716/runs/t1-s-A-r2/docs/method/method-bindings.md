...
|---|---|---|---|
| **DoR gate** | spec-readiness check before decompose | `definition-of-ready.md` + `keel check-ready` | `docs/method/definition-of-ready.md` + `./bin/keel check-ready spec-<date>-<slug>.md` (Part A, `--structure-only` in the author loop before a pre-mortem is recorded); Part B is a blind pre-mortem run by a reviewer who did not author the spec, using `docs/method/pre-mortem-prompt.md`, then `./bin/keel check-ready spec-<date>-<slug>.md` (no flag) to confirm both halves |
