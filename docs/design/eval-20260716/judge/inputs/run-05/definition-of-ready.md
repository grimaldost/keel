...
- [ ] Every `path:line` anchor resolves (file + line exist) and any quoted note — the
...
A6 each `path:line` anchor: fail unless file exists, line in range, and any quoted note (the backticked token right after the anchor) matches
