...
| `pre-mortem-profiles.md` | The profile-selected sheets, dispatched by the header's `Profile:` field (A14): `measurement` for an experiment, `data-pipeline` for a spec whose deliverable is data — population before schema, a gate per heterogeneity axis, a constructed pilot, an exact write-side reconciliation and a DISCRIMINATING read check, pinned clocks evaluated against every read predicate, and every staged literal checked against the vocabulary that owns it. A `code` spec pays for neither. |
