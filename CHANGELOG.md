...
resolved the coordinate and stopped there, so a row naming `§1` passed while its anchor sat on a
blank line, on a heading, or three sections away — two programmes measured the consequence (8 of
35 rows wrong, then 90 of 125) while `check-ready` printed `OK`, and both repaired it with a
throwaway difflib script, because `re-anchor`'s snippet-driven repair needs a snippet the shipped
template calls optional. Both halves close here: detection in A12, repair in a second re-anchor
strategy that reads the tree instead of the snippet. Four smaller field rows ride with them — the
messages that name what is wrong and never the shape they want, a verdict that could not name the
copy that produced it, a build twin that resolved as if it were source, and the release loop's
nine hand edits.

### Added

- **A12 asserts where the row says the fold landed** (`check_ready.py`, `_ledger_landing`). A
  fold-ledger row's confirmation must resolve onto a NON-BLANK line — true of any target file — and,
  when it points into the spec that carries the ledger and the row names a `§N`, must land inside
  that section, with the failure naming the section it actually landed in. Both halves of the
  assertion were already in the module (`_split_top_sections`, `_section_id`) and the row has
  carried its own `Target section` cell since 0.5.0; nothing read it. A row naming no `§N` makes no
  claim and stays silent (verify-when-present), and a `§N` the spec does not declare remains A8's
  dangling reference. **A section's own heading line counts as inside it**: run against this
  repository's own design specs, the rule accused three of them of a failure they do not have —
  their ledgers anchor every row at the heading of the section that row names, deliberately and
  consistently — and a check whose fires include its author's own convention reports nothing. The
  same run found two rows that ARE wrong (a 0.13.0 row confirmed against a blank `README.md:5`, and
  one whose own text reads "misattributed retirement to §11" landing outside every numbered
  section) plus six historical anchors into files that have moved since; `docs/design/` is
  maintainer-local (ADR-0012), no gate runs over it, and those records are left as the certified
  history they are rather than edited under their own hashes.
- **`keel re-anchor <spec> --by-content <git-ref>`** (`reanchor.py`, `docs/cli-reference.md`): the
  repair for the rows the template actually emits. Instead of locating a row by its optional
  snippet, it reads the line the anchor cited AS OF that ref and repoints to wherever that content
  sits now — so an unbackticked, snippet-less confirmation cell is repairable, and a range moves
  both ends, which the snippet strategy refuses by construction. The refusals stay the boundary: a
  line inside a hunk changed or deleted since the ref is reported by name and left alone, as is a
  row whose snippet would no longer match the line its content moved to. Ledger-only by default,
  so the certified hash does not move.
- **`scripts/bump_version.py <x.y.z>`**, which writes every version site from `VERSION_SITES` —
  the registry that now lives inside the consistency test that enforces it, so the bump and the
  gate cannot hold different lists. `src/keel/templates/core/spec-template.md` joins that registry
  as its eighth site: it was enforced only by `test_core_variants`, where a stale stamp fails as a
  strict-subset error whose message says nothing about versions. The newest CHANGELOG heading and
  `uv.lock` are reported, never written — rewriting the heading's number would rename the previous
  release, and the lock is regenerated by `uv lock`.
- **Two adversarial-corpus mutants for the new A12 arm** (`A12-blank-line-confirmation`,
  `A12-row-lands-in-another-section`), each a one-edit delta firing exactly A12.

### Changed

- **`check-ready` prints one summary line on both exit paths** (consumer-affecting)
  (`cli.py:_emit`): `keel <version> — N checks applicable, M fired`. Two plugin caches were live on
  one machine and no keel output distinguished them; a session ran 0.18.0 by `uvx` while 0.18.1 was
  installed and reconstructed that fact from paths afterwards, filing a finding whose premise was
  false. The counts answer the other half: an `OK` from a gate with three applicable checks read
  exactly like an `OK` from a thorough one. A gate that reports no probes still names the version.
- **Six rejections print the grammar they parse**, instead of only what was wrong with what they
  read: B1's Operator (`- **Operator:** <name>` on its own line, the label carrying no
  parenthesis — the trap that made a well-formed record invisible), B1's Verdict and Reviewer
  lines, B2's `PREMORTEM-VERDICT: CERTIFIED` artifact line, A12's ledger row shape, A5's
  concept→module header (`| Concept | Module / file |`, named on the row that can be a header,
  since a header missing one of the two words is read as data and fails as "the word Module is not
  a path"), and W1's kit stamp (a bare `x.y.z`; a `v` prefix reads as no stamp at all). Six field
  grammars were discovered by iteration in one rite, each costing its own run-read-fix cycle.
- **Build-output trees join the refusal class vendored trees already occupied** (`_COPY_TREES`,
  formerly `_VENDOR_DIRS`): `target/`, `build/`, `dist/` and `_build/`. A dbt seed's compiled
  `target/run/…` twin resolved by unique basename and passed as a WARN — a file nobody edits,
  uniquely matched, and always the wrong answer. Same message, widened to name a generated copy as
  well as a vendored one.
- **The version-consistency test names every disagreeing site in one message**, rather than the
  first assertion that happens to fire. A patch bump was measured at five rounds of
  run-test/read-failure/fix-one-site; the message now carries the whole disagreement, including a
  site whose pattern matched nothing at all.
- **Prose displacements, per the body-budget rule.** The DoR reference block's A12 line (60 of its
  61-word cap) states the landing rule by displacing "rather than a fixed column" and compressing
  "a row wider than its own header is a column break and fails as one"; its W3 line names the
  build-output tree. The spec-template's fold-ledger note (contract notes at 499 of 500) states the
  same rule by displacing "recorded against a real line, not that it is right" and "Header only and
  A12 dozes" — the second is already carried by R1's own reference line. CONTRIBUTING's release
  paragraph is rewritten around the script rather than the hand list.
