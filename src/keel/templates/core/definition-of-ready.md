...
W3 (warn) an anchor that does not resolve as written but whose basename matches exactly ONE repo file OUTSIDE a vendored or build-output tree resolves to that file and warns, naming the expansion; ambiguity, no match, or a match only inside such a copy tree still fails, naming the twin (A6/A11/A12)
